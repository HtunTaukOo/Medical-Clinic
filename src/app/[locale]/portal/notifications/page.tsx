...
export default async function PortalNotificationsPage() {
  ...
  const now = new Date();
  const t = await getTranslations("portal.notificationsPage");

  const CATEGORY_META: Record<NotificationCategory, { label: string; icon: typeof Bell; badgeClass: string }> = {
    APPOINTMENT: { label: t("categoryAppointments"), icon: CalendarClock, badgeClass: "bg-blue-100 text-blue-700" },
    LAB_RESULT: { label: t("categoryResults"), icon: FlaskConical, badgeClass: "bg-emerald-100 text-emerald-700" },
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            <span className="font-medium text-primary">{t("unreadCount", { count: unreadCount })}</span>{" "}
            {t("unreadSuffix")}
          </p>
        </div>
        {unreadCount > 0 && (
